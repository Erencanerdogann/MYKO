...
void CUser::ReqHandleUserDatabaseRequest(Packet& pkt)
{
	...
	case ProcDbType::UpdateAccountKnightCash:
	{
		...
		if (cashamount && tlamount) {
			log.append(string_format("%d Balance and %d Cash has been loaded into your account.", tlamount, cashamount));
			twon = true;
		}
		...
		if (!twon && cashamount)
			...
			log.append(string_format("%d Balance ", tlamount));
		...
		if (id != -1) {
			...
			if (pUser) g_pMain->SendHelpDescription(pUser, string_format("Current Player Cash: %d - Balance: %d", m_nKnightCash, m_nTLBalance));
		}
	}
	break;
	...
	case ProcDbType::PusGiftSendLetter:
	{
		...
		if (m_nKnightCash < itemprice) return;

		m_nKnightCash -= itemprice;
		g_pMain->SendHelpDescription(this, string_format("You have gift purchased %s for %d Knight Cash.", itemdata.m_sName.c_str(), itemprice).c_str());
...
void CUser::ReqAccountLogIn(Packet & pkt)
{
	...
	// SHA256 hash migration: password check moved to ACCOUNT_LOGIN SP (line 926)
	// Pre-check removed — SP handles SHA256 hash comparison securely
	// pUserTbInfo.accountpass now holds the hash (for kick-out flow)

	std::string AccountID = strAccountID;
	STRTOUPPER(AccountID);

	/* ===== RECONNECT FIX (Session 66) — Eski session sessiz dismiss, yeni session devam ===== */
	if (disconnect)
	{
		...
		if (pUser && pUser->GetSocketID() != GetSocketID()) {
			...
			// SendLoginFailed kaldirildi: eski session kesildi, yeni session devam ediyor
		}
	}

	// Enhanced Ban System: IP + Account ban kontrolu (BANNED_LIST tablosundan)
	{
		std::string strIP = GetRemoteIP();
		int8 banResult = g_DBAgent.CheckBanStatus(strAccountID, strIP);
		if (banResult > 0)
		{
			printf("[BAN] Login rejected: Account=%s, IP=%s, BanType=%d\n",
				strAccountID.c_str(), strIP.c_str(), banResult);
			Packet banPkt(WIZ_LOGIN);
			banPkt << int8(-1);
			Send(&banPkt);
			return;
		}
		...
		if (pUser) {
			...
			if (pUser->GetSocketID() != GetSocketID() && AccountID == mapAccountID) {
				...
				if (find != g_pMain->m_accountNameMap.end())
					g_pMain->m_accountNameMap.erase(AccountID);
				g_pMain->m_accountNameLock.unlock();
				// SendLoginFailed + return kaldirildi: eski session kesildi, yeni session devam ediyor
			}
		}

		m_strAccountID = strAccountID;
		g_pMain->AddAccountName(this);
		g_pMain->ResetLoginAttempt(GetRemoteIP()); // Basarili login — sayaci sifirla
	...
	g_pMain->RecordLoginFail(GetRemoteIP()); // Basarisiz login — sayaci artir
	LOG_LOGIN("[LOGIN_FAIL] Account=%s IP=%s Result=%d", strAccountID.c_str(), GetRemoteIP().c_str(), dbresult);
...
void CUser::ReqAllCharInfo(Packet & pkt)
{
	...
	case AlreadyCharName:
	{
		...
		ServerUniteSelectingCharNameErrorOpcode NameChangeResult = ServerUniteSelectingCharNameErrorOpcode::CannotCharacterID;
		uint8 Result = g_DBAgent.CharacterSelectLoginChecking(GetAccountName(), sOldCharID);
		if (Result == 1) //25.11.2023 bu paketle istedikleri karekterin nickini değiştirebiliyorlardı fixlendi
...
void CUser::ReqCreateNewChar(Packet & pkt)
{
	...
	if (sonuc == 0) {
		...
		bool isISTIRAP = g_DBAgent.LoadAutoClanInfo(strCharID, newClass, newLevel, newClanID, newFame, newLoyalty, newMontlyLoyalty);
		if ((isISTIRAP && newFame == 5) && (newClanID == 1 || newClanID == 15001)) {
...
void CUser::ReqSkillDataSave(Packet & pkt)
{
	...
	if (sCount == 0 || (sCount * sizeof(uint32)) > sizeof(buff))
		return;
...
{
	// FIX: Only skip save if character was never loaded (level 0).
	// Previously !m_deleted check blocked save on sudden DC — removed.
	if (m_bLevel == 0) {
		m_deleted = false;
	...
	if (isInGame()) {
		...
		// B10 fix: Parti cleanup ONCE — UserInOut(INOUT_OUT) oncesinde yapilmali
		if (isInParty()) {
	...
	LOG_LOGIN("[LOGOUT] User=%s Account=%s IP=%s Zone=%d Level=%d Nation=%d",
		GetName().c_str(), GetAccountName().c_str(), GetRemoteIP().c_str(), GetZoneID(), GetLevel(), GetNation());
	...
	// CHI-23: Cikis yapinca IP sayacini azalt — sadece isInGame() ile eslestir
	// IncrementIPUserCount sadece GAME_STATE_INGAME ataninca yapiliyor
	if (isInGame())
...
{
	// FIX: Double-save protection — minimum 1 second interval
	if ((UNIXTIME - m_lastSaveTime) < 1)
		return;
...
void CUser::ReqSetLogInInfo(Packet & pkt)
{
	...
	LOG_LOGIN("[LOGIN] User=%s Account=%s IP=%s Zone=%d Level=%d Nation=%d",
		GetName().c_str(), GetAccountName().c_str(), GetRemoteIP().c_str(), GetZoneID(), GetLevel(), GetNation());
