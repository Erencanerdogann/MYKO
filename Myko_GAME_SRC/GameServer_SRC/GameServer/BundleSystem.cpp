...
void CUser::ISTIRAPAutoLooter(_LOOT_BUNDLE* pBundle) {
	...
	for (int i = 0; i < NPC_HAVE_ITEM_LIST; i++) {
		...
		if (pBundle->Items[i].nItemID == ITEM_GOLD) {
			...
			if (pReceiver->isDead() || pReceiver->isMerchanting()) {
				pReceiver = GetLootUser(pBundle, pBundle->Items[i]);
			...
			// Overflow-safe addition: check before adding to prevent uint16 wrap-around
			if ((uint32)pDstItem->sCount + (uint32)pBundle->Items[i].sCount > MAX_ITEM_COUNT)
...
void CUser::ItemGet(Packet& pkt)
{
	...
	if (nItemID == ITEM_GOLD) {
		...
		// Overflow-safe addition: check before adding to prevent uint16 wrap-around
		if ((uint32)pDstItem->sCount + (uint32)pBundle->Items[SlotID].sCount > MAX_ITEM_COUNT)
			pDstItem->sCount = MAX_ITEM_COUNT;
		else
