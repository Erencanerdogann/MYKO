...
void AdiniFerihaKoydum::tKnightLogger()
{
	while (_running[(uint8)dbreqtype::Logger]) {
		...
		// H26: Table name whitelist to prevent SQL injection
		{
			static const std::set<std::string> validLogTables = {
				"NPC_SHOPPING", "CHAT", "LOGIN", "LOGOUT", "DISCONNECT",
				"ITEMS_RECEIVED", "ITEMS_LOST", "MERCHANT_CREATION", "MERCHANT_SHOPPING",
