...
void CUser::LotteryJoinFunction()
{
	...
	/*foreach_stlmap(itr, g_pMain->m_RimaLotteryUserInfo)
	{
		...
		if (pUserInfo->Name == GetName())
		{
			pkt.clear();
			pkt.Initialize(XSafe);
			pkt << uint8(XSafeOpCodes::LOTTERY) << uint8(3) << uint8(0) << std::string("Zaten Kat�ld�n�z");
			Send(&pkt);
			return;
		}
	}*/
	...
	{
		std::string strCheckID = GetName();
		STRTOUPPER(strCheckID);
		_RIMA_LOOTERY_USER_INFO *pCheckInfo = g_pMain->m_RimaLotteryUserInfo.GetData(strCheckID);
		if (pCheckInfo != nullptr)
		{
			pkt.clear();
			pkt.Initialize(XSafe);
			pkt << uint8(XSafeOpCodes::LOTTERY) << uint8(3) << uint8(0) << std::string("Zaten Katildiniz");
			Send(&pkt);
			return;
		}
	}
	...
	// Collect items to rob, then rob all at once
	struct _roblist { uint32 id; uint32 count; };
	std::vector<_roblist> robItems;
	for (int i = 0; i < 5; i++) {
		uint32 ItemID = g_pMain->pLotteryProc.nReqItem[i];
		uint32 ItemCount = g_pMain->pLotteryProc.nReqItemCount[i];
		if (!ItemID || !ItemCount)
		...
		if (!CheckExistItem(ItemID, ItemCount)) {
			if (nReqGold > 0) GoldGain(nReqGold);
			return;
		}
		robItems.push_back({ ItemID, ItemCount });
	}
	for (auto& r : robItems) {
		if (!RobItem(r.id, r.count)) {
			if (nReqGold > 0) GoldGain(nReqGold);
			return;
		}
	}
	...
	if (pUserInfo != nullptr) {
		...
		if (!g_pMain->m_RimaLotteryUserInfo.PutData(strUserID, pList)) { // strname �zerinden yap�cak
			pkt.clear();
			pkt.Initialize(XSafe);
...
void CGameServerDlg::LotteryEventTimer()
{
	...
	if (pLotteryProc.EventTime != 0 && pLotteryProc.TimerControl)
	{
		//kalan notice
		uint32 RemainingTime = 0;
		if (pLotteryProc.EventProcessTime > (uint32)UNIXTIME)
			RemainingTime = pLotteryProc.EventProcessTime - (uint32)UNIXTIME;
		if (RemainingTime > 0)
		{
