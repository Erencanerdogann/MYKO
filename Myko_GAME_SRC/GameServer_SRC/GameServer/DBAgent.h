...
public:
	...
	bool InsertCurrentUser(std::string & strAccountID, std::string & strCharID, const std::string & strServerIP = "127.0.0.1", const std::string & strClientIP = "127.0.0.2");
...
#pragma endregion
	...
	bool VerifyPassword(std::string accountid, std::string password);
	...
	OdbcConnection* GetGameDB();

private:
	OdbcConnection *m_GameDBPool[GAME_DB_POOL_SIZE];
	OdbcConnection *m_AccountDB, *m_LogDB;
