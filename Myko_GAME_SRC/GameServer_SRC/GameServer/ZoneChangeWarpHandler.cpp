...
bool CUser::ZoneChange(uint16 sNewZone, float x, float z, int16 eventroom /*= -1*/, bool check /*= true*/, bool arrest_summon /*= false*/, bool v_eventlogout)
{
	...
	// B9 fix: Zone gecisi sirasinda GenieStop/PetOnDeath gibi fonksiyonlarin
	// SendToRegion ile eski zone'a broadcast yapmasini engelle
	m_bIsZoneChanging = true;
	...
	LOG_ZONE("[ZONE] User=%s From=%d To=%d X=%.0f Z=%.0f Level=%d",
		GetName().c_str(), oldZoneForLog, sNewZone, x, z, GetLevel());

	ZoneOnlineRewardChange();

	SetRegion(GetNewRegionX(), GetNewRegionZ());

	m_bIsZoneChanging = false; // B9 fix: Zone gecisi tamamlandi
