...
void CUser::VIPhouseProcess(Packet & pkt)
{
	...
	// Lock order: inventoryLock first, then vipWarehouseLock (prevents deadlock)
	std::lock_guard<std::recursive_mutex> inventoryGuard(m_inventoryLock);
	std::lock_guard<std::recursive_mutex> vipLock(m_vipWarehouseLock);
	...
	if (m_bVIPStorageVaultExpiration < (uint32)UNIXTIME)
		...
		uint16 reference_pos = 48 * page;
		if (page > 1 || reference_pos + bSrcPos >= VIPWAREHOUSE_MAX
			|| reference_pos + bDstPos >= VIPWAREHOUSE_MAX) 
			return UseVipKeyError(opcode, 2);

		_ITEM_DATA* pSrcItem = &m_sVIPWarehouseArray[reference_pos + bSrcPos];
		_ITEM_DATA* pDstItem = &m_sVIPWarehouseArray[reference_pos + bDstPos];
