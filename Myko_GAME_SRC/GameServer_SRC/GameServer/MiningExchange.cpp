...
void CUser::MiningDropTester(uint8 WeaponType, uint8 Hour)
{
	...
	foreach(itr, pDropList)
	{
		...
		Send(&result);
		GiveItem("Drop Test", itr->ItemID, itr->ItemCount, true, 1); // 29.10.2020 Mining Drop Testten Gelen Itemler Art�k 1 Gun Sureyle Gelecek
...
void CUser::FishingDropTester(uint8 WeaponType, uint8 Hour)
{
	...
	foreach(itr, pDropList)
	{
		...
		result << uint8(ChatType::GENERAL_CHAT) << uint8(0) << uint16(-1) << uint8(0) << Command;
		Send(&result);
		GiveItem("Fishing Drop Test", itr->ItemID, itr->ItemCount, true, 1); // 29.10.2020 Fishing Drop Testten Gelen Itemler Art�k 1 Gun Sureyle Gelecek
		//GiveItem(itr->ItemID, itr->ItemCount);
	}
...
void CUser::MiningExchange(uint16 ID)
{
	...
	if (MiningExchangeList.size() > 0)
	{
		...
		if (!CheckExistItem(pMiningExchange->nOriginItemNum, 1))
		{
			g_pMain->m_MiningExchangeArray.m_lock.unlock();
			return;
		}
		if (!RobItem(pMiningExchange->nOriginItemNum, 1))
		{
			g_pMain->m_MiningExchangeArray.m_lock.unlock();
			return;
		}
