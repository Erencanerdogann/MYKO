...
void CKnightsManager::ReqKnightsRemove(CUser *pUser, Packet & pkt)
{
	...
	// Only leader or vice chief can remove members
...
void CKnightsManager::ReqKnightsViceChief(CUser* pUser, Packet & pkt)
{
	...
	// Prevent duplicate vice chief assignment
	if (pKnights->m_strViceChief_1 == strTarget || pKnights->m_strViceChief_2 == strTarget || pKnights->m_strViceChief_3 == strTarget)
		return;
...
void CKnightsManager::ReqKnightsRegisterSymbol(CUser *pUser, Packet & pkt)
{
	...
	if (sSymbolSize > sizeof(pKnights->m_Image)) sSymbolSize = sizeof(pKnights->m_Image);
