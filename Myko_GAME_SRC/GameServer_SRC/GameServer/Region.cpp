...
void CRegion::Add(CUser* pUser)
{
	...
	std::unique_lock<std::shared_mutex> lock(m_lockUserArray);
...
void CRegion::Add(CNpc* pNpc)
{
	...
	std::unique_lock<std::shared_mutex> lock(m_lockNpcArray);
...
void CRegion::Add(CBot * pBot)
{
	...
	std::unique_lock<std::shared_mutex> lock(m_lockBotArray);
