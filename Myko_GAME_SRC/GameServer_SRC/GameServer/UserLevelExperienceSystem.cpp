...
void CUser::ExpChange(std::string descp, int64 iExp, bool bIsBonusReward)
{
	...
	if (iExp > 0)
	{
		if (!bIsBonusReward)
		{
			...
			if (FinalExp > INT64_MAX / 2)
			FinalExp = INT64_MAX / 2;
...
void CUser::LevelChange(uint8 level, bool bLevelUp)
{
	...
	// B6 fix: Kendine ayrica gonder — Send_Region timing sorunu yuzunden kendi efekti gorunmuyordu
	Send(&result);
	g_pMain->Send_Region(&result, GetMap(), GetRegionX(), GetRegionZ(), this, GetEventRoom());
	if (isInParty())
	{
...
bool CUser::JackPotExp(int64 iExp)
{
	...
	int64 m_bExpEvent = (int64)iExp * (int64)xrand;
	int64 maxExp = g_pMain->GetExpByLevel(GetLevel(), GetRebirthLevel());
	if (maxExp > 0 && m_bExpEvent > maxExp) m_bExpEvent = maxExp;
