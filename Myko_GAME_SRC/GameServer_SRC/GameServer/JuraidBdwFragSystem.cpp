...
void CUser::BDWAltarScreenAndPlayerPointChange()
{
	...
	if (bNation == Nation::ELMORAD && e_count)
	{
		...
		pRoomInfo->m_AltarElmoScoreBoard += 10; //25.11.2023 m_AltarElmoScoreBoard, m_AltarKarusScoreBoard
	}
	else if (bNation == Nation::KARUS && k_count)
	{
		pRoomInfo->m_KarusScoreBoard += 10 * k_count;
		pRoomInfo->m_AltarKarusScoreBoard += 10; //25.11.2023 m_AltarElmoScoreBoard, m_AltarKarusScoreBoard
	}
	...
	if (pRoomInfo->m_bWinnerNation == 0) {
		...
		if (pRoomInfo->m_ElmoScoreBoard >= TotalPoint)
			...
			pRoomInfo->m_bWinnerNation = (uint8)Nation::KARUS;
		//25.11.2023 m_AltarElmoScoreBoard, m_AltarKarusScoreBoard start
		if (pRoomInfo->m_AltarElmoScoreBoard >= 50)
			pRoomInfo->m_bWinnerNation = (uint8)Nation::ELMORAD;
		else if (pRoomInfo->m_AltarKarusScoreBoard >= 50)
			pRoomInfo->m_bWinnerNation = (uint8)Nation::KARUS;
		//25.11.2023 m_AltarElmoScoreBoard, m_AltarKarusScoreBoard end
...
void CUser::BDWUpdateRoomKillCount()
{
	...
	if (pRoomInfo->m_bWinnerNation == 0) {
		...
		uint16 TotalPoint = 0;
		if (t_count >= 16) TotalPoint = 150;
		else if (t_count >= 10) TotalPoint = 100;
		else if (t_count >= 5) TotalPoint = 80;
		else TotalPoint = 50;
