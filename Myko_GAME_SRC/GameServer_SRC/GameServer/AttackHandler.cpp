...
void CUser::Attack(Packet & pkt)
{
	...
	if (tid < 0)
	{
		LOG_HACK("[PACKET_INVALID] User=%s Type=Attack tid=%d negative", GetName().c_str(), tid);
		return;
	}

	// bType validation: only known attack types
	if (bType > 2)
	{
		LOG_HACK("[PACKET_INVALID] User=%s Type=Attack bType=%u invalid", GetName().c_str(), bType);
		return;
	}
	...
	/*if (m_range && pTarget)
	{
		...
		if (mesafe > (float)m_range + 3.0f)
			return;
	}*/

	damage = GetDamage(pTarget);

	// G9: Server-side damage validation — cap + log suspicious values
	if (damage < 0)
		damage = 0;
	else if (damage > 32000)
	{
		LOG_HACK("[DAMAGE_CAP] User=%s tid=%d damage=%d capped to 32000", GetName().c_str(), tid, damage);
		damage = 32000;
	}
	...
	if (!pTarget->isPlayer())
	{
		...
		else if (TO_NPC(pTarget)->m_OrgNation == 3 && TO_NPC(pTarget)->GetType() != NPC_BORDER_MONUMENT) // R Atack Kapatldi 27.09.2020 - BDW Altar istisna
		{
			damage = 0;
		}
...
void CUser::Regene(uint8 regene_type, uint32 magicid /*= 0*/)
{
	...
	else // we're respawning using a resurrect skill.
	{
		...
		if (GetZoneID() != ZONE_UNDER_CASTLE)
		{
			...
			// EXP kaybi SADECE mob olumunde olur (OnDeathKilledNpc m_iLostExp set eder, whoKilledMe=-1).
			// PvP olumunde EXP kaybi YOK (m_iLostExp=0) -> geri verilecek bir sey yok. KO kurali dogru.
			if (m_sWhoKilledMe == -1)
	...
	// Olunce tum Type4 buff/debuff temizlenir, sonra sadece buff'lar (SC vs.) geri yuklenir
	InitType4(false);
	// Buff state degiskenlerini sifirla (debuff kalintisi kalmasin)
	m_sMaxHPAmount = 0;
	m_sMaxMPAmount = 0;
	m_sACAmount = 0;
	m_sACPercent = 100;
	...
	m_bAddDiseaseR = 0;
	m_bAddPoisonR = 0;
	m_bBlockCurses = false;
	m_bReflectCurses = false;
	InitType3();
