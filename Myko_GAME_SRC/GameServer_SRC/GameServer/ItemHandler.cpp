...
bool CUser::GiveItem(std::string placeowned, uint32 itemid, uint16 count, bool send_packet /*= true*/, uint32 Time, _giveitempusinfo pPus, uint8 nSession)
{
	...
	pItem->nNum = itemid;
	if (pItem->sCount + count > MAX_ITEM_COUNT)
...
void CUser::InventorySystem(Packet & pkt)
{
	...
	case ITEM_INVEN_TO_COSP:
		...
		{
			if (INVENTORY_COSP + bDstPos + 2 >= INVENTORY_TOTAL)
				goto fail_return;
...
#define MAX_DISPLAY 8
	...
	foreach(ISTIRAPNumberOne, m_exchangelist) {

		uint32 nItemID = ISTIRAPNumberOne->ItemID;
		uint32 nCount = ISTIRAPNumberOne->ItemCount;
...
void CUser::ItemRemove(Packet & pkt)
{
	...
	SetUserAbility();
	SendItemWeight();
	UserDataSaveToAgent(); // Item silindikten sonra aninda kaydet
	result << uint8(1);
	Send(&result);
