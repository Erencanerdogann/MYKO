...
void CUser::CindirellaLogOut(bool exitgame) {
	...
	if (pCindWar.gainexp) {
		m_iExp += pCindWar.gainexp;
		int64 maxExp = g_pMain->GetExpByLevel(GetLevel(), GetRebirthLevel());
		if (maxExp > 0 && m_iExp > maxExp) m_iExp = maxExp;
	}
	if (pCindWar.gainnoah) {
		if (m_iGold + pCindWar.gainnoah > COIN_MAX) m_iGold = COIN_MAX;
		else m_iGold += pCindWar.gainnoah;
	}
...
void CGameServerDlg::CindirellaTimer() {
	...
	if (!remtime) {
		...
		std::sort(e_scoreboard.begin(), e_scoreboard.end(), [](auto const& a, auto const& b) { return a.killcount != b.killcount ? a.killcount > b.killcount : a.deadcount < b.deadcount; });
		std::sort(k_scoreboard.begin(), k_scoreboard.end(), [](auto const& a, auto const& b) { return a.killcount != b.killcount ? a.killcount > b.killcount : a.deadcount < b.deadcount; });
