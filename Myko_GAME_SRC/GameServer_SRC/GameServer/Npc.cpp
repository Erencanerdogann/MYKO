...
void CNpc::RecvAttackReq(int nDamage, uint16 sAttackerID, AttributeType attributeType /*= AttributeNone*/)
{
	...
	if (pAttacker && pAttacker->isPlayer() && MyDamage > 0)
	{
		...
		else
		{
			...
			// Cap damage list at 30 entries — evict lowest damage dealer
			if (m_DamagedUserList.size() > 30) {
				auto minIt = m_DamagedUserList.begin();
				for (auto it = m_DamagedUserList.begin(); it != m_DamagedUserList.end(); ++it) {
					if (it->second.Damage < minIt->second.Damage)
						minIt = it;
				}
				m_DamagedUserList.erase(minIt);
			}
		}
		m_damageuserListLock.unlock();
	}
...
void CNpc::Dead(Unit* pKiller /*= nullptr*/, bool open_bundle /*= true*/)
{
	...
	m_ISTIRAPDelay = m_sRegenTime + UNIXTIME2;
	m_bFirstLive = false;
	m_byDeadType = 100;
...
void CNpc::SendAttackRequest(int16 tid)
{
	...
	if (pAttacker->isNPC() && pTarget->isPlayer() && TO_USER(pTarget)->m_OrgNation == 3)
		return;
	...
	if (!bos_check) {
		if (GetType() != NPCTYPE_MONSTER || m_bIsEventNpc) {
			...
			if (nTileX < 0 || nTileZ < 0) {
				printf("ByNoise| #### Npc-SetLive() Fail : nTileX=%d, nTileZ=%d #####\n", nTileX, nTileZ);
				return false;
			}
...
//		pRegion->m_lockNpcArray.lock_shared();
//		ZoneNpcArray cm_RegionNpcArray = pRegion->m_RegionNpcArray;
//		pRegion->m_lockNpcArray.unlock_shared();  // sağdaki şeyi açtım durdurdum
//		foreach(itr, cm_RegionNpcArray)
//		{
//			int nNpcid = (*itr);
//			if (nNpcid < NPC_BAND)
...
void CNpc::SendExpToUserList()
{
	...
	m_damageuserListLock.lock();
	auto m_copyUserlist = m_DamagedUserList;
	m_damageuserListLock.unlock();
...
void CNpc::GiveNpcHaveItem(Unit *pKiller)
{
	...
	if (robinActive)
	pUser->ISTIRAPAutoLooter(pBundle);
}
