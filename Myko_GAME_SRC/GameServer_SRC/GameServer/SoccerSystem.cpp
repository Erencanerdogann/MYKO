			...
			&& pSoccerEvent.m_SoccerTime < 600))
	{
		...
		foreach_stlmap(itr, g_pMain->m_arNpcThread)
		{
			...
			foreach_stlmap(btr, pNpcThread->m_arNpcArray)
			{
				...
				uint8 ballState = pNpc->isInSoccerEvent();
				if (ballState >= (uint8)TeamColour::TeamColourBlue
					&& ballState <= (uint8)TeamColour::TeamColourOutside)
				{
					if (ballState == (uint8)TeamColour::TeamColourRed
						|| ballState == (uint8)TeamColour::TeamColourBlue)
					{
						if (ballState == (uint8)TeamColour::TeamColourRed)
						...
						TempleSoccerEventGool(ballState);
					}

					pNpc->SendInOut(INOUT_OUT, pNpc->GetX(), pNpc->GetZ(), 0.0f);
					pNpc->SendInOut(INOUT_IN, 672.0f, 160.0f, 0.0f);
				}
				bBallFound = true;
				break;
			}
