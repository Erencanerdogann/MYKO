...
*/
CUser::CUser(uint16 socketID, SocketMgr* mgr) : KOSocket(socketID, mgr, -1, 1024 * 32, 1024 * 32), Unit(UnitType::UnitPlayer) {
	//Initialize();
}
#pragma endregion
...
void CUser::Initialize()
{
	...
	m_lastrattacktime = 0;
	m_packetCount = 0;
	m_packetCountResetTime = 0;
	m_lastMoveBroadcast = 0;

	m_bCharacterDataLoaded = false;
	m_bKingVoted = false;
	m_nVIPPasswordFailCount = 0;
	m_nLastTournamentKillTime = 0;
	...
	// m_bDisconnected = false; // RECONNECT PARK
	isBottomSign = false;
	m_bIsChicken = false;
	...
	m_bIsZoneChanging = false;
	m_lastZoneChangeTime = 0;
	...
	m_GameMasterChaosEvent = 0;
	m_GameMasterBdwEvent = 0;
	m_GameMasterJrEvent = 0;
	m_GameMasterRoyalEvent = 0;
	m_GameMasterBeefEvent = 0;
...
{
	/* ===== RECONNECT PARK (Session 25) — Orijinal OnDisconnect'e dönüldü =====
	 * Reconnect sistemi rafa kaldırıldı. Aşağıdaki grace period, map cleanup,
	 * m_bDisconnected flag vs. devre dışı. Tekrar aktif etmek için:
	 * C:\temp\MYKO\yedekler\RECONNECT_BACKUP\ ve C:\temp\Chip\RECONNECT_PARK.md
	 *
	 * Offline merchant/genie koruması orijinalde var, dokunulmadı.
	 * ========================================================================= */

	// Offline merchant/genie aciksa logout yapma — karakter dunyada kalir (ORİJİNAL)
	if (IsOffCharacter() && GetOffMerchantTime() > 0)
	{
		KOSocket::OnDisconnect();
		return;
	}

	// CHI-21: DC aninda isInGame() false olabilir (Genie zone change vb.)
	// ReqUserLogOut isInGame() kontrolu altinda parti temizliyor — buradan onceden yap
	if (!isInGame() && isInParty()) {
...
std::string PacketSend(uint8 Opcode)
{
	...
	case WIZ_COMPRESS_PACKET:
		...
		break;
	case 0xA0:
		return PacketHeader = "LEGACY_0xA0";
		break;
	case WIZ_LOGIN:
...
bool CUser::HandlePacket(Packet & pkt)
{
	...
	// Packet flood protection: max 150 packets per second
	m_packetCount++;
	if (UNIXTIME2 > m_packetCountResetTime) {
		m_packetCount = 1;
		m_packetCountResetTime = UNIXTIME2 + 1000;
	}
	if (m_packetCount > (uint32)g_ServerConfig.PacketRateLimit()) {
		LOG_HACK("Packet flood: %s (%s) %u pkt/sec IP=%s", GetName().c_str(), GetAccountName().c_str(), m_packetCount, GetRemoteIP().c_str());
		goDisconnect("Packet flood detected", __FUNCTION__);
		return false;
	}
...
void CUser::UpdateCheckSkillTime() {
	...
	if (UNIXTIME2 > m_tLastType4SkillCheck) {
		...
		Type4Duration();
		// B1 FIX: Zone gecisi sirasinda SavedMagic expire kontrolu atla
		// Zone yuklenmesi sirasinda kisa sureli bufflar siliniyor, RecastSavedMagic bos calisiyor
...
void CUser::UpdateCheckTime() {
	...
	if ((UNIXTIME - m_lastSaveTime) >= PLAYER_SAVE_INTERVAL
		&& (UNIXTIME % PLAYER_SAVE_INTERVAL) == (m_socketID % PLAYER_SAVE_INTERVAL)) {
...
void CUser::Update()
{
		...
		&& UNIXTIME > off_merchantcheck)
	{
		...
		{
			SetOffCharacter(_choffstatus::DEACTIVE);
			// Timer bitti ve socket kapalıysa (DC durumu) → logout yap
			if (!IsConnected())
				return LogOut();
			return;
		}
	...
	// DC olmuş ama offline merchant değilse buraya gelmemeli — güvenlik kontrolü
	if (!IsConnected() && !IsOffCharacter())
		return LogOut();
	...
	foreach(itr, partyUsers) {
		...
		if (nFinalExp > 0) {
			...
			if (iExp > 0 && !pUser->JackPotExp(iExp))
				pUser->ExpChange(string_format("monster (2)exp protoid=%d", protoid), iExp);
		}
...
void CUser::SpeedHackUser()
{
	...
	{
		LOG_HACK("[SPEEDHACK] User=%s Speed=%d MaxSpeed=%d", GetName().c_str(), m_sSpeed, nMaxSpeed);
		goDisconnect("Couldn't pass speed control. speedhack check", __FUNCTION__);
	}
}

bool CUser::CheckInvisibility(CUser* ptarget) {
	// NOTE: Invisibility check intentionally disabled — all subsequent code is dead
	return true;
...
void CUser::SendNewDeathNotice(Unit* pKiller)
{
	...
	uint8 myZone = GetZoneID();
	uint16 myEventRoom = GetEventRoom();
	uint16 killerID = pKillerUser->GetID();
	uint16 victimID = GetID();
	int16 killerPartyID = pKillerUser->isInParty() ? pKillerUser->GetPartyID() : -1;
	uint16 deathX = (uint16)GetX();
	uint16 deathZ = (uint16)GetZ();

	// Onceden 3 tip paket olustur (killtype 1, 2, 3)
	// Her kullanici icin ayri paket olusturmak yerine 3 paket paylas
	Packet pktSelf(XSafe, uint8(XSafeOpCodes::DeathNotice));     // killtype=1 (killer/victim)
	pktSelf.SByte();
	pktSelf << uint8(1) << pKillerUser->GetName().c_str() << GetName().c_str() << deathX << deathZ;

	Packet pktParty(XSafe, uint8(XSafeOpCodes::DeathNotice));    // killtype=2 (party member)
	pktParty.SByte();
	pktParty << uint8(2) << pKillerUser->GetName().c_str() << GetName().c_str() << deathX << deathZ;

	Packet pktOther(XSafe, uint8(XSafeOpCodes::DeathNotice));    // killtype=3 (diger)
	pktOther.SByte();
	pktOther << uint8(3) << pKillerUser->GetName().c_str() << GetName().c_str() << deathX << deathZ;

	// MAX_USER dongusu yerine aktif session map kullan — sadece online kullanicilar
	...
	for (auto itr = sessions.begin(); itr != sessions.end(); ++itr)
	{
		...
		if (pUser->GetZoneID() != myZone || pUser->GetEventRoom() != myEventRoom)
			continue;

		if (pUser->GetID() == killerID || pUser->GetID() == victimID)
			pUser->Send(&pktSelf);
		else if (killerPartyID >= 0 && pUser->GetPartyID() == killerPartyID)
			pUser->Send(&pktParty);
		else
			pUser->Send(&pktOther);
	}
...
void CUser::ResetWindows()
{
	...
	// B9 fix: Zone change sirasinda genie'yi durdur — yoksa genie stop + zone change paketleri carpisiyor
	if (isGenieActive())
		GenieStop();
...
void CUser::HandleHelmet(Packet & pkt)
{
	...
	{
		// B2 fix: HEAD eklendi — cospre gizlendiginde normal helmet gorunumu guncellenmiyordu
		uint8 equippedItems[] = { HEAD, BREAST, LEG, GLOVE, FOOT };
...
{
	// v5.0 (patron S134): m_savedMagicMap'e artik SADECE SC scroll'lari giriyor (MagicInstance:4000
	//   bShouldPersist = isLockableScroll). Dolayisiyla recast zaten yalnizca SC'yi geri getirir;
	//   ayri SC-only filtreye gerek yok. SC-disi buff (EXP/premium/skill/kitap) saved'e hic girmez ->
	//   olum/zone/relog hepsinde gider. SC kalir.
	m_savedMagicLock.lock();
...
void CUser::goDisconnect(std::string log /*= ""*/, std::string funcname /*= ""*/) {
	if (log.size() && funcname.size()) {
		LOG_DISCONNECT("[KICK] User=%s Account=%s IP=%s Zone=%d Level=%d Func=%s Reason=%s",
			GetName().c_str(), GetAccountName().c_str(), GetRemoteIP().c_str(), GetZoneID(), GetLevel(),
			funcname.c_str(), log.c_str());

		Packet newpkt(0);
		newpkt << std::string("DISCONNECT");
		newpkt << string_format("'%s','%s',GETDATE(),'%s','%s'",
...
#pragma region CUser::ClanNationChange()

void CUser::ClanNationChange()
{
	uint8_t sNation;
	uint16_t CharClass = 0, nNum = 0, AccountCount = 0, bError = 0;
	std::string strCharID1, strCharID2, strCharID3, strCharID4;
	uint8_t bCurrentNation = GetNation();

	/*if (!CheckExistItem(800400000))
	{
		bError = 7;
		return;
	}*/
	...
	CKnights* pKnights;
	std::string pKnightsName;
	foreach_stlmap(itr, g_pMain->m_KnightsArray)
	{
		if (itr->second->GetID() == GetClanID())
		{
			pKnights = g_pMain->GetClanPtr(itr->first);
			pKnightsName = pKnights->GetName();
			break;
		}
	}
	...
	std::vector<std::string> NationChange;
	{
		Guard loock(pKnights->m_arKnightsUser.m_lock);
		foreach_stlmap_nolock(i, pKnights->m_arKnightsUser)
		{
			...
			NationChange.push_back(p->strUserName);
		}
	}

	foreach(itr, NationChange)
	{
		...
		uint32_t sClass;
		if (pTUser->GetZoneID() != ZONE_MORADON)
			pTUser->ZoneChange(ZONE_MORADON, 0.0f, 0.0f);

		if (GetNation() == (uint8_t)Nation::KARUS && pTUser->GetClass() < 200)
		{
			sClass = pTUser->GetClass() + 100;
			sNation = 2;
		}
		else if (GetNation() == (uint8_t)Nation::ELMORAD && pTUser->GetClass() > 200)
		{
			sClass = pTUser->GetClass() - 100;
			sNation = 1;
		}

		pTUser->m_bRace = pTUser->GetNewRace();
		pTUser->m_sClass = sClass;
		pTUser->m_bNation = sNation;
		pTUser->m_nHair = 0;
		pTUser->m_bFace = 0;
		pTUser->m_bKnights = pTUser->GetClanID();

		pTUser->SendMyInfo();

		pTUser->UserInOut(INOUT_OUT);
		pTUser->SetRegion(GetNewRegionX(), GetNewRegionZ());
		pTUser->UserInOut(INOUT_WARP);
	...
	g_DBAgent.ChangeClanNation(pKnights->GetName());
	//RobItem(800400000);
}
