...
bool CUser::TempleJoinEvent(int16 sEvent)
{
	...
	case EventOpCode::TEMPLE_EVENT_JURAD_MOUNTAIN:
		...
		Send(&result);
		if (GetNation() == (uint8)Nation::KARUS) { if (g_pMain->pTempleEvent.KarusUserCount < 10000) g_pMain->pTempleEvent.KarusUserCount++; }
		else { if (g_pMain->pTempleEvent.ElMoradUserCount < 10000) g_pMain->pTempleEvent.ElMoradUserCount++; }
