...
void CUser::NpcEvent(Packet & pkt)
{
	...
	if (isDead()
		...
		|| isMining()
		|| isFishing())
		//|| m_bSelectMsgFlag != 0)  // BAKIM FIX GERI ALINDI: NPC etkileşimini bozuyor
		return;
	...
	case NPC_TOURNAMENT_MANAGER:
		// S115 — Tournament Manager NPC: dialog tamamen Lua'ya tasindi
		// (Quests/32756_TournamentManager.lua + QUEST_HELPER migration 113).
		// Lua engine NPC tikla event'inde QUEST_HELPER'a bakarak otomatik
		// 32756_TournamentManager.lua dosyasini calistirir, EVENT=50000 girisi.
		// Eski C++ chat handler artik gereksiz — eger Lua engine yoksa NPC sessiz kalir.
		break;
	...
	if (type == 1 && pNpc->m_iSellingGroup != 249000)
	{
		for (int i = 0; i < purchased_item_count; i++) {
			...
			if (pItems[i].ITEMID != 0) {
				...
				/*Bus d�pesinin engellendi�i yer*/
				...
				total_price += (uint32)((uint64)pTable.m_iBuyPrice * pItems[i]._ICOUNT);
				if (total_price > COIN_MAX) { errorCode = 2; goto fail_return; }
					...
					if (pMap != nullptr)
					{
						...
						case ZONE_ARENA:
						{
							...
							if (g_pMain->pSiegeWar.sMoradonTariff == 0)
								...
								transactionPrice += tax1;
							// Guard: prevent unsigned underflow wraparound
							if (transactionPrice > COIN_MAX) transactionPrice = 0;
						}
						break;
						case ZONE_DELOS:
...
void CUser::SendRepurchaseMsg(bool isRefreshed /*= false*/)
{
	...
	foreach_stlmap(itr, m_sDeletedItemMap) {
		...
		nPrice = pItem.m_iBuyPrice * pItemDeleted->sCount * 30;  //��pe at�lan itemin geri al�nma de�erini buras� hesapl�yor.
		if (nPrice > COIN_MAX)
			nPrice = COIN_MAX;
