...
void CUser::SendMyInfo(bool after /*after = false*/)
{
	...
	if (GetLevel() < 30 && !ChickenStatus && GAME_SOURCE_VERSION == 1098)		// V2 i�in buras� aktif olucak.
...
/*			if (pItemTable->isPetItem()) //  istedi
	...
	// Renkli pm i�in geli�tirildi krala �zel renk felan 27.09.2020 start
	if (isGM())
	{
	...
	// Renkli pm i�in geli�tirildi krala �zel renk felan 27.09.2020 end
	m_bGearSkills = true;
}
...
void CUser::GetUserInfo(Packet & pkt)
{
	...
	pkt << GetClanID() << uint8(isInPKZone() == true ? uint8(0) : GetFame()); // -> [uint8(isInPKZone() == true ? uint8(0) : GetFame()] <- soldaki kod pk b�lgelerinde clan baskan� normal clan uyesi gibi g�z�k�r
	...
	if (pKnights == nullptr)
	{
		if (isKing())
			...
			//pkt << uint32(0) << uint16(0) << uint8(0) << uint16(GetNation() == KARUS ? 97 : 98) << uint32(0) << uint8(0); //Userlerin �rk pelerini g�stermesi kald�r�ld�
	...
	else
	{
		...
		if (pKnights->isInAlliance() && pMainClan != nullptr && pAlliance != nullptr)
		{
			if (!isKing())
			{
				if (pMainClan->isCastellanCape() && pMainClan->m_CastTime >= (uint32)UNIXTIME) //03.04.2024
				{
					if (pAlliance->sMainAllianceKnights == pKnights->GetID())
						pkt << pMainClan->m_castCapeID << pMainClan->m_bCastCapeR << pMainClan->m_bCastCapeG << pMainClan->m_bCastCapeB << uint8(0); //03.04.2024
					else if (pAlliance->sSubAllianceKnights == pKnights->GetID())
						pkt << pMainClan->m_castCapeID << pKnights->m_bCapeR << pKnights->m_bCapeG << pKnights->m_bCapeB << uint8(0); //03.04.2024
					else if (pAlliance->sMercenaryClan_1 == pKnights->GetID() || pAlliance->sMercenaryClan_2 == pKnights->GetID())
						pkt << pMainClan->m_castCapeID << uint32(0);
					else
						pkt << pMainClan->m_castCapeID << pMainClan->m_bCapeR << pMainClan->m_bCapeG << pMainClan->m_bCapeB << uint8(0); //03.04.2024
				}
				else {
					if (pAlliance->sMainAllianceKnights == pKnights->GetID())
						pkt << pMainClan->GetCapeID() << pMainClan->m_bCapeR << pMainClan->m_bCapeG << pMainClan->m_bCapeB << uint8(0); //03.04.2024
					else if (pAlliance->sSubAllianceKnights == pKnights->GetID())
						pkt << pMainClan->GetCapeID() << pKnights->m_bCapeR << pKnights->m_bCapeG << pKnights->m_bCapeB << uint8(0); //03.04.2024
					else if (pAlliance->sMercenaryClan_1 == pKnights->GetID() || pAlliance->sMercenaryClan_2 == pKnights->GetID())
		...
		{
			if (!isKing()) {
				if (pKnights->isCastellanCape() && pKnights->m_CastTime >= (uint32)UNIXTIME) //03.04.2024
					pkt << pKnights->m_castCapeID << pKnights->m_bCastCapeR << pKnights->m_bCastCapeG << pKnights->m_bCastCapeB << uint8(0); //03.04.2024
				else
					pkt << pKnights->GetCapeID() << pKnights->m_bCapeR << pKnights->m_bCapeG << pKnights->m_bCapeB << uint8(0); //03.04.2024
			}
			else {
	...
	//bool civicv = GetLevel() < 30 && !ChickenStatus && V2TAHT == 1;									// v2myko i�in buras� aktif olucak.
	bool civicv = GetLevel() < 30 && !ChickenStatus && GAME_SOURCE_VERSION == 1098;				// 1098 i�in buras� aktif olucak.
	if (GetLevel() < 30 && !ChickenStatus && GAME_SOURCE_VERSION == 1098)		// V2 i�in buras� aktif olucak.
