...
public:	
	...
	static uint32 THREADCALL Timer_BotCleanup(void* lpParam);
	...
	// XP/coin/NP events — CHI-20: uint16 yapildi, max 1000 destegi
	uint16 m_byExpEventAmount, m_byCoinEventAmount, m_byNPEventAmount, m_byDropEventAmount;
	...
	time_t m_MaintenanceKickTime;
	void   MaintenanceTimer();

	// Kufur engelleme sistemi
	bool m_bCensorEnabled;
	std::vector<std::string> m_CensorWords;
	...
	std::recursive_mutex m_LoginAttemptLock;
	std::map<std::string, LoginAttempt> m_LoginAttemptMap;
	bool CheckLoginAttempt(const std::string &strIP);
	void RecordLoginFail(const std::string &strIP);
	void ResetLoginAttempt(const std::string &strIP);

	// CHI-23: IP bazi online kullanici sayaci (multi-box limiti)
	static constexpr uint8 MAX_USERS_PER_IP = 3; // ayni IP den max 3 karakter online
	std::recursive_mutex m_IPUserCountLock;
	std::map<std::string, uint8> m_IPUserCountMap;
	bool CheckIPUserLimit(const std::string &strIP);
	void IncrementIPUserCount(const std::string &strIP);
	void DecrementIPUserCount(const std::string &strIP);
...
public:
	...
	COMMAND_HANDLER(HandleBetLimitsCommand);           // S115 ekonomi — /betlimits MIN MAX
