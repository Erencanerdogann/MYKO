...
static void LogLuaError(const char* filename, const char* message)
{
	FILE* f = fopen("logs/lua_errors.log", "a");
	if (f == nullptr)
		return;
	...
	fprintf(f, "[%s] FILE: %s | %s\n", timebuf, filename ? filename : "?", message ? message : "");
	fclose(f);
}
...
DEFINE_LUA_FUNCTION_TABLE(g_globalFunctions,
	...
	MAKE_LUA_FUNCTION(SendNationChangeClan)
...
bool CLuaScript::Initialise()
{
	...
	// Expose scripts to Lua libraries (safe subset only)
	luaL_openlibs(m_luaState);

	// Sandbox: kaldirilan kutuphaneler Lua scriptlerinden erisim yapamaz
...
bool CLuaEngine::ExecuteScript(CUser * pUser, CNpc * pNpc, int32 nEventID, int8 bSelectedReward, const char * filename)
{
	...
	if (itr == m_scriptMap.end())
	{
		...
		// Release the lock before compilation (I/O operation - avoid holding lock during file read)
		m_lock->unlock();
		...
		// Execute the script - CLuaScript::ExecuteScript has its own lock (m_luaScript.m_lock)
		// so we don't need m_lock here (script map caching is disabled anyway)
	...
	else
	{
		...
		// Copy the bytecode so we can release m_lock before executing
		BytecodeBuffer bytecode = itr->second;
		m_lock->unlock();
...
bool CLuaScript::ExecuteScript(CUser * pUser, CNpc * pNpc, int32 nEventID, int8 bSelectedReward, const char * filename, BytecodeBuffer & bytecode)
{
	...
	case LUA_ERRRUN:
		...
		printf("NPC ID: %d\n", pNpc != nullptr ? pNpc->GetProtoID() : 0);
		printf("-\n");
		LogLuaError(filename, "RUNTIME ERROR");
		break;
	...
	case LUA_ERRMEM:
		...
		printf("NPC ID: %d\n", pNpc != nullptr ? pNpc->GetProtoID() : 0);
		printf("-\n");
		LogLuaError(filename, "MEMORY ERROR");
		break;
	...
	case LUA_ERRERR:
		...
		printf("NPC ID: %d\n", pNpc != nullptr ? pNpc->GetProtoID() : 0);
		printf("-\n");
		LogLuaError(filename, "ERROR HANDLER FAILED");
		break;
	...
	default:
		...
		printf("NPC ID: %d\n", pNpc != nullptr ? pNpc->GetProtoID() : 0);
		printf("-\n");
		LogLuaError(filename, "UNKNOWN ERROR");
		break;
	}
	...
	{
		const char* luaMsg = lua_to<const char *>(m_luaState, -1);
		printf("ERROR: [%s] The following error was provided.\n", filename);
		printf("MESSAGE: %s\n", luaMsg);
		printf("-\n");
		LogLuaError(filename, luaMsg);
	}
...
{
	// Wait until any executing script finishes, then release.
	Guard lock(m_lock);
	// Cleanup will continue as normal in destructor.
}
