...
void CUser::BifrostPieceProcess(Packet & pkt)
{
	...
	for (int i = SLOT_MAX; i < SLOT_MAX + HAVE_MAX; i++) {
		_ITEM_DATA* pSlot = GetItem(i);
		if (pSlot != nullptr && pSlot->nNum == 0 && ++bFreeSlots >= ITEMS_IN_EXCHANGE_GROUP) break;
	}
	...
	if (offset == 0) return BifrostPieceSendFail(errorcode);
