...

	{
		...
		if (!SelectHumanList.empty()) {
			for (int i = 0; i < MAX_SELECTING_USER; i++) {
				...
				SelectingElmoradUser[i] = myrand(0, HumanIndex - 1);
		...
		if (!SelectKarusList.empty()) {
			...
			for (int i = 0; i < MAX_SELECTING_USER; i++) {
				...
				RLandSelectingKarusUser[i] = myrand(0, KarusIndex - 1);
				for (int kontrol = 0; kontrol < i; kontrol++) {
...
void CUser::NewWantedEventLoqOut(Unit* pKiller)
{
	...
	if ((Nation)GetNation() == Nation::ELMORAD) {
		...
		pMain.m_elmolist.erase(GetSocketID());
	}
	else {
		auto itr = pMain.m_karuslist.find(GetSocketID());
		if (itr == pMain.m_karuslist.end()) return;
		pMain.m_karuslist.erase(GetSocketID());
	}
...
void CUser::WantedRegister(Packet& pkt)
{
	...
	if ((Nation)GetNation() == Nation::ELMORAD) pMain.m_elmolist.insert(std::make_pair(GetSocketID(), _WANTED_USER(this, true)));
	else pMain.m_karuslist.insert(std::make_pair(GetSocketID(), _WANTED_USER(this, true)));
