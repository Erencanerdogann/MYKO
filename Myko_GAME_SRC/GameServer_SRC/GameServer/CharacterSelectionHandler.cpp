...
void CUser::SelCharToAgent(Packet & pkt)
{
	...
	if (g_pMain->m_bMaintenanceMode && !isGM()) { result << uint8(0); Send(&result); return; }
...
void CUser::SelectCharacter(Packet & pkt)
{
	...
	if ((pUser && pUser->m_bSelectedCharacter))
		return goDisconnect("ByNoise | DC [1].", __FUNCTION__);
	...
	case 1:
	{
		...
		{
			LOG_HACK("[LEVEL_FIX] %s Level %d > MaxLevel %d, auto-fix", GetName().c_str(), GetLevel(), g_pMain->m_byMaxLevel);
			m_bLevel = g_pMain->m_byMaxLevel;
		}
	...
	case 4:
	{
		...
		if (strCharID1.length() != 0)
		{
			if (g_DBAgent.CharacterSelectPrisonCheck(strCharID1, ZoneID))
			{
				...
				{
					result << uint8(PrisonCharacter) << strCharID1 << "ByNoise";
					Send(&result);
					return;
				}
			}
		...
		if (strCharID2.length() != 0)
		{
			if (g_DBAgent.CharacterSelectPrisonCheck(strCharID2, ZoneID))
			{
				if (ZoneID == ZONE_PRISON)
				{
					result << uint8(PrisonCharacter) << strCharID2 << "ByNoise";
					Send(&result);
					return;
				}
			}
		...
		if (strCharID3.length() != 0)
		{
			if (g_DBAgent.CharacterSelectPrisonCheck(strCharID3, ZoneID))
			{
				if (ZoneID == ZONE_PRISON)
				{
					result << uint8(PrisonCharacter) << strCharID3 << "ByNoise";
					Send(&result);
					return;
				}
			}
		...
		if (strCharID4.length() != 0)
		{
			if (g_DBAgent.CharacterSelectPrisonCheck(strCharID4, ZoneID))
			{
				if (ZoneID == ZONE_PRISON)
				{
					result << uint8(PrisonCharacter) << strCharID4 << "ByNoise";
					Send(&result);
					return;
				}
			}
...
void CUser::GameStart(Packet & pkt)
{
	...
	else if (opcode == 2)
	{
		...
		// CHI-23: Ayni IP den cok fazla karakter girisini engelle (GM muaf)
		if (!isGM() && !g_pMain->CheckIPUserLimit(GetRemoteIP()))
		{
			g_pMain->SendHelpDescription(this, "Too many characters online from your IP address.");
			return goDisconnect("IP user limit exceeded.", __FUNCTION__);
		}

		ZoneOnlineRewardStart();
		m_state = GameState::GAME_STATE_INGAME;
		g_pMain->IncrementIPUserCount(GetRemoteIP());
		g_pMain->AddCharacterName(this);
		UserInOut(INOUT_RESPAWN);

		// RECONNECT PARK — RestorePartyFromGrace();
		// RestorePartyFromGrace();
