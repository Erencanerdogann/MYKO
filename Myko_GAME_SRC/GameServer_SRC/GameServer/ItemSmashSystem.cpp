...
void CUser::ItemDisassemble(Packet & pkt)
{
	...
	for (int i = 0; i < rollcount; i++) {
		...
		if (offset[i] == 0) return SendItemDisassembleFail(SmashExchangeError::SmashItem);
	...
	for (int i = 0; i < rollcount; i++) {
		...
		auto* pISTIRAP = g_pMain->m_ItemExchangeCrashArray.GetData(nExchangeID[i]);
		if (pISTIRAP) pCrashItem = *pISTIRAP;
	...
	if (!ItemSmashSystemRobItemCheck(SLOT_MAX + npos, nitemid, 1)) //25.11.2023 narkide k�rd�r�lan itemin kaybolmamas� fix
		return SendItemDisassembleFail(SmashExchangeError::SmashItem);
	...
	Send(&result);
	UserDataSaveToAgent(); // Item kirma sonrasi aninda kaydet
}

bool CUser::ItemSmashSystemRobItemCheck(uint8 bPos, uint32 nItemID, uint16 sCount) //25.11.2023 narkide k�rd�r�lan itemin kaybolmamas� fix
{
