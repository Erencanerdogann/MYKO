...
void CUser::ReqLetterSend(Packet & pkt)
{
	...
	if (pItem != nullptr)
	{
		if (pItem->nNum == nItemID && pItem->nSerialNum == Serial)
			...
			bResult = -1; // Serial mismatch — fail, don't return success
	}
	else
		bResult = g_DBAgent.SendLetter(m_strUserID, strRecipient, strSubject, strMessage, bType, pItem, nCoins);
...
void CUser::ReqLetterGetItem(Packet & pkt)
{
	...
	if (nItemID != 0) {
		pItem->nNum = nItemID;
		if (pItem->sCount + sCount > MAX_ITEM_COUNT) { result << int8(-1); Send(&result); return; }
		pItem->sCount += sCount;
		...
		if (nExpirationTime) {
			if (nExpirationTime > 2592000) nExpirationTime = 2592000; // max 30 gun
			pItem->nExpirationTime = int32(UNIXTIME) + nExpirationTime;
		}
		// Letterda S�reli Item Gonderme   
