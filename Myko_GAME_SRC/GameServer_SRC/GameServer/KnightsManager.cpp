...
void CKnightsManager::KnightsJoin(CUser *pUser, Packet & pkt)
{
	...
	result << uint8(KnightsPacket::KNIGHTS_JOIN) << bResult;
	pUser->Send(&result);
}
#pragma endregion
...
void CKnightsManager::KnightsAllMember(CUser *pUser)
{
	...
	if (bResult == 1)
	{
		...
		pktSize = ((uint16)result.size() - pktSize) + 6; //2024
		result.put(2, pktSize); //2024
		result.put(4, count); //2024
		result.put(10 + pKnights->m_strClanNotice.size(), count); //2024
	}
	pUser->Send(&result);
}
...
void CKnightsManager::ListTop10Clans(CUser *pUser)
{
	...
	Packet result(WIZ_KNIGHTS_PROCESS, uint8_t(KnightsPacket::KNIGHTS_TOP10));
	result << uint16_t(1);
	...
	for (int nation = KARUS_ARRAY; nation <= ELMORAD_ARRAY; nation++)
	{
		...
		foreach_stlmap(itr, g_pMain->m_KnightsRatingArray[nation])
		{
			CKnights* pKnights = g_pMain->GetClanPtr(itr->second->sClanID);
			if (pKnights != nullptr && (pKnights->m_byNation == nation + 1))
				result << pKnights->m_sIndex << pKnights->m_strName << pKnights->m_sMarkVersion << int16_t(i++);
...
void CKnightsManager::AddUserDonatedNP(int index, std::string & strUserID, uint32 nDonatedNP, bool isAddClanFund /*= false*/)
{
	...
	if (pKnightUser->nDonatedNP + nDonatedNP > LOYALTY_MAX) pKnightUser->nDonatedNP = LOYALTY_MAX;
	else pKnightUser->nDonatedNP += nDonatedNP;
}
