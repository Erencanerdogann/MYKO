...
bool CMagicProcess::UserRegionCheck(Unit * pSkillCaster, Unit * pSkillTarget, _MAGIC_TABLE pSkill, int radius, short mousex /*= 0*/, short mousez /*= 0*/)
{
	...
	case MORAL_AREA_ALL:
		...
		if ((TO_USER(pSkillCaster)->isInArena() && TO_USER(pSkillTarget)->isInArena())
			|| (TO_USER(pSkillCaster)->isInPVPZone() && TO_USER(pSkillTarget)->isInPVPZone())
			|| (TO_USER(pSkillCaster)->isInTempleEventZone() && TO_USER(pSkillTarget)->isInTempleEventZone())
			|| (TO_USER(pSkillCaster)->isInTempleQuestEventZone() && TO_USER(pSkillTarget)->isInTempleQuestEventZone()))
			goto final_test;

		// Players cant attack other players in the safety area.
		if (TO_USER(pSkillCaster)->isInEnemySafetyArea() && TO_USER(pSkillTarget)->isInOwnSafetyArea())
			return false;
		break;
...
bool CMagicProcess::GrantType4Buff(_MAGIC_TABLE pSkill, _MAGIC_TYPE4 *pType, Unit * pCaster, Unit *pTarget, bool bIsRecastingSavedMagic /*= false*/)
{
	...
	if (pTarget->m_buffMap.find(pType->bBuffType) != pTarget->m_buffMap.end())
	{
		if (bIsRecastingSavedMagic)
			pTarget->m_buffMap.erase(pType->bBuffType);
		else
			return false;
	}
	...
	case BUFF_TYPE_WEIGHT:
		if (pTarget->isPlayer())
		{
			...
			TO_USER(pTarget)->SetUserAbility(); //21.01.2024
			TO_USER(pTarget)->SendItemWeight(); //21.01.2024
		}
		break;
