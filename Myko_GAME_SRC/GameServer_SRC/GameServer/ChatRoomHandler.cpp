...
void CUser::ChatRoomLeave(Packet & pkt)
{
	...
	bool bIsAdmin = (pRoom->isAdministrator(GetName()) == 2);

	// Decrement user count before potential room deletion (avoid use-after-free)
	if (pRoom->m_sCurrentUser > 0)
...
void CUser::ChatRoomKickOut(uint16 userID)
{
	...
	// User not found in room — send error
	if (!bFound)
	{
		result << uint8(2);
		Send(&result);
	...
	// Notify the kicked user to reset their chat room index
	CUser * pKicked = g_pMain->GetUserPtr(userID);
	if (pKicked != nullptr)
		pKicked->m_ChatRoomIndex = -1;
