...
int CNpcThread::_Engine() {
	try
	{
		...
		while (_running) {
			deleted.clear();
			if (g_pMain->npcthreadreload) { Sleep(100); continue; }
...
bool CGameServerDlg::_CreateNpcThread() {
	...
	foreach(ISTIRAPOne, mZoneList) {
		CNpcThread* pNpcThread = new CNpcThread((*ISTIRAPOne)->m_nZoneNumber);
		if (!m_arNpcThread.PutData((*ISTIRAPOne)->m_nZoneNumber, pNpcThread)) { delete pNpcThread; return false; }

		std::string threadid = std::to_string((*ISTIRAPOne)->m_nZoneNumber);
		if ((*ISTIRAPOne)->m_nZoneNumber < 10) threadid = string_format("0%d", (*ISTIRAPOne)->m_nZoneNumber);
		++counter;
...
bool CGameServerDlg::_LoaderSpawnCallBack(OdbcCommand* _dbCommand) {
	...
	for (uint16 j = 0; j < bNumNpc; j++)
	{
		...
		pNpc->InitPos();
		if (sSid == JURAID_BRIDGE_SSID) //21.01.2024
			iRange = 0;
		...
		if (bDotCnt > 0) {
			...
			for (int l = 0; l < bDotCnt; l++) {
				...
				index += 4;
				pNpc->m_PathList.pPatternPos[l].x = (int16)SafeAtoi(std::string(szX), -32768, 32767);
				pNpc->m_PathList.pPatternPos[l].z = (int16)SafeAtoi(std::string(szZ), -32768, 32767);
			}
		}
