...
void CUser::GivePremium(uint8 bPremiumType, uint16 sPremiumTime, bool minute)
{
	...
	else
	{
		pPremium->bPremiumType = bPremiumType;
		if (minute) {
			uint32 addTime = 60 * sPremiumTime;
			if (addTime / 60 != sPremiumTime || pPremium->iPremiumTime + addTime < pPremium->iPremiumTime) return;
			pPremium->iPremiumTime = pPremium->iPremiumTime + addTime;
		} else {
			if (sPremiumTime > 49) sPremiumTime = 49; // max 49 days to prevent signed int overflow in 24*60*60*sPremiumTime
			uint32 addTime = 24 * 60 * 60 * sPremiumTime;
			if (addTime / 86400 != sPremiumTime || pPremium->iPremiumTime + addTime < pPremium->iPremiumTime) return;
			pPremium->iPremiumTime = pPremium->iPremiumTime + addTime;
		}
	}
...
void CUser::GiveSwitchPremium(uint8 bPremiumType, uint16 sPremiumTime) //17.12.2020 Switch Premium Yukler Yuklemez 3 �n� birden goruntuluyecek sekilde yeni komut eklendi
{
	if (bPremiumType <= 0
		|| bPremiumType > 13
		|| sPremiumTime <= 0
