...
void CGameServerDlg::UserInOutForMe(CUser *pSendUser)
{
	...
	static const uint16 MAX_VISIBLE_USERS = 200;
	...
	foreach_region(x, z)
	{
		if (user_count >= MAX_VISIBLE_USERS)
			break;
...
void CGameServerDlg::BotInOutForMe(CUser* pSendUser)
{
	...
	static const uint16 MAX_VISIBLE_BOTS = 100;
	...
	foreach_region(x, z)
	{
		if (user_count >= MAX_VISIBLE_BOTS)
			break;
		...
		pRegion->m_lockBotArray.lock_shared();
		if (pRegion->m_RegionBotArray.size() <= 0) {
			pRegion->m_lockBotArray.unlock_shared();
			continue;
		}
...
void CGameServerDlg::GetRegionBotIn(C3DMap *pMap, uint16 region_x, uint16 region_z, Packet & pkt, uint16 & t_count, uint16 nEventRoom)
{
	...
	foreach(itr, cm_RegionUserBotArray)
	{
		...
		// Suresi dolan botlari pakete yazma, sadece silme listesine ekle
...
void CGameServerDlg::GetRegionUserList(C3DMap* pMap, uint16 region_x, uint16 region_z, Packet & pkt, uint16 & t_count, uint16 nEventRoom, bool SurUpdate /* = false */, CUser *pExcept /* = nullptr*/)
{
	...
	static const uint16 MAX_REGION_USERS = 200;
...
{
	// S3: Snapshot pattern — lock suresi kisaltildi
	std::vector<CUser*> targets;
	{
		std::lock_guard<std::recursive_mutex> lock(m_socketMgr.GetLock());
		SessionMap& sessions = m_socketMgr.GetActiveSessionMap();
...
{
	// S3: Snapshot pattern — lock suresi kisaltildi, Send() lock disinda yapilir
	std::vector<CUser*> targets;
	{
		std::lock_guard<std::recursive_mutex> lock(m_socketMgr.GetLock());
		SessionMap& sessions = m_socketMgr.GetActiveSessionMap();
	...
	// Send disinda lock tutulmaz — diger thread ler session map e erisebilir
	for (CUser* pUser : targets)
		pUser->Send(pkt);
}
#pragma endregion
...
void CGameServerDlg::SendNoticeWindAll(std::string notice, uint32 color, uint8 ZoneID)
{
	...
	std::vector<CUser*> targets;
	{
		...
		{
			CUser* pUser = static_cast<CUser*>(itr->second);
			if (pUser == nullptr || !pUser->isInGame() || (ZoneID > 0 && ZoneID != pUser->GetZoneID()))
				continue;
			targets.push_back(pUser);
		}
	}
