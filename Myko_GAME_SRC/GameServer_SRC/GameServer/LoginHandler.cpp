...
void CUser::KickOutProcess(Packet & pkt)
{
	...
	// Security: only authenticated users can trigger kick-out
	if (m_strAccountID.empty())
		return;

	// Security: players can only kick their own account (server-switch flow)
	if (account != m_strAccountID)
		return;

	CUser* pUser = g_pMain->GetUserPtr(account, NameType::TYPE_ACCOUNT);
	if (pUser != nullptr)
	{
		bool bOfflineMerchant = (pUser->m_offmerchanttime > 0);
		if (bOfflineMerchant)
...
void CUser::ReqKickOutProcess(Packet& pkt)
{
	...
	// SHA256 migration: compare hash instead of plaintext
	// getmypassword returns the stored hash, passid is the raw encoded password
	// We need to verify via DB-side hash comparison
	if (!g_DBAgent.VerifyPassword(accountid, passid))
		return;
...
void CUser::LoginProcess(Packet & pkt)
{
	...
	// Brute force korumasi — IP bazli rate limit
	if (!g_pMain->CheckLoginAttempt(GetRemoteIP()))
		return SendLoginFailed(-1);
