...
void MagicInstance::Run()
{
	...
	if (pSkill.isnull())
		return SendSkillFailed(-1, "R1_skill_null");
	...
	if (pSkillCaster == nullptr)
		return SendSkillFailed(-1, "R2_caster_null");
	...
	if (pSkillCaster->isPlayer()) {
		...
		if (pSkill.bMoral == 7 || pSkill.bMoral == 10) {
			...
			if(pSkillCaster->isInSpecialEventZone()
				...
				return SendSkillFailed(-1, "R3_event_zone_closed");

			if (g_pMain->isCindirellaZone(pUser->GetZoneID())
				&& g_pMain->pCindWar.isON()
				&& !g_pMain->pCindWar.isStarted()
				&& !g_pMain->pSpecialEvent.opened)
				return SendSkillFailed(-1, "R4_cind_not_started");
		}

		_ISTIRAPinfo& p = pUser->pUserMagicUsed.status;
		if (AnimatedSkill() && pSkill.bType[0] == 2 && pSkill.bCastTime && (MagicOpcode)bOpcode == MagicOpcode::MAGIC_CASTING) {
			...
			if (p.skillid && p.skillid != pSkill.iNum && UNIXTIME2 - p.m_castusetime < 500)
				return SendSkillFailed(-1, "R5_cast_spam_500ms");
	...
	if (sTargetID != -1 && !pSkillTarget)
		return SendSkillFailed(-1, "R7_target_null");
	...
	if (pSkillTarget)
	{
		if (pSkillTarget->isNPC())
		{
			...
			if ((pSkill.bMoral == MORAL_ENEMY || pSkill.bMoral == MORAL_AREA_ENEMY) && (pSkillCaster->isPlayer() && TO_NPC(pSkillTarget)->m_OrgNation == 3))
				return SendSkillFailed(-1, "R8_npc_nation3");
		}
	}
	...
	if (pSkillCaster->isPlayer()) 
	{
		...
		if (sTargetID == -1)
		{
			// Validate target position on map
			if (pSkillCaster->GetMap() && !pSkillCaster->GetMap()->IsValidPosition(float(sData[0]) / 10.0f, float(sData[2]) / 10.0f, 0.0f))
				return SendSkillFailed(-1, "R9_invalid_pos");
			...
			if (!noblock && mesafe > (pSkill.iUseItem == 391010000 ? (float)500 : (float)SkillRange))
				return SendSkillFailed(-1, "R10_out_of_range");
		}
		...
		if (pSkillCaster->isPlayer() && AnimatedSkill())
		{
			...
			if (returnfail)
				return SendSkillFailed(-1, "R14_animated_move_fail");
		}
	...
	if (pSkillCaster->isPlayer() && pSkillTarget) 
	{
		...
		if (TO_USER(pSkillCaster)->isGenieActive()
			...
			return SendSkillFailed(-1, "R15_genie_pvp_zone");
	...
	if (Failed && !no_casting_failed)
		return SendSkillFailed(-1, "R16_prereq_fail");
	...
	if (Failed && !no_casting_failed)
		return SendSkillFailed(-1, "R17_usercancast_fail");
...
SkillUseResult MagicInstance::UserCanCast()
{
	...
	if (pSkill.iNum >= 300000 && pSkill.iNum <= 400000)
	{
		if (pSkillCaster->isPlayer())
		{
			if (TO_USER(pSkillCaster)->GetZoneID() == ZONE_CHAOS_DUNGEON &&  nSkillID != 490221 || TO_USER(pSkillCaster)->GetZoneID() == ZONE_CHAOS_DUNGEON && nSkillID != 490332)
				UCC_FAIL("chaos_dungeon_skill");
		}
	}

	if (pSkillCaster->isPlayer() && pSkillCaster->isInTempleEventZone() && !g_pMain->pTempleEvent.isAttackable && TO_USER(pSkillCaster)->isEventUser())
		UCC_FAIL("temple_not_attackable");
	...
	if (pSkillCaster->isPlayer())
	{
		...
		{
			bool no_check = false;
			// Arrow skills: only skip mana check during FLYING (projectile in-flight), NOT during EFFECTING (damage phase)
			if ((pSkill.bType[0] == 2 || pSkill.bType[1] == 2) && bOpcode == (uint8)MagicOpcode::MAGIC_FLYING)
				no_check = true;
		...
		if (pSkill.iNum == 210004 || pSkill.iNum == 209004 || pSkill.iNum == 109004 || pSkill.iNum == 110004)
		{
			...
			if (pSkillCaster && pSkillCaster->isPlayer() && pSkillTarget->isPlayer() && TO_USER(pSkillCaster)->GetPartyID() != TO_USER(pSkillTarget)->GetPartyID())
				return SkillUseResult::SkillUseFail;
		}
...
bool MagicInstance::CheckSkillClass(int iskillclass) {
	...
	if (!pSkillCaster || !pSkillCaster->isPlayer()) return false;
	CUser* pCaster = TO_USER(pSkillCaster);
				...
				if (itr != pCaster->m_CoolDownList.end()) {
					...
					if ((pCaster->isWarrior() || pCaster->isPriest() || pCaster->GetZoneID() == ZONE_CHAOS_DUNGEON)
						...
						if (itr->second.time > 0 && pSkill.sReCastTime * 102 > (UNIXTIME2 - itr->second.time)) {
							//itr->second.time = UNIXTIME2; //2024
							pCaster->m_lockCoolDownList.unlock();
							return SkillUseResult::SkillUseNoFunction;
						}
					}
				...
				if (pSkill.bType[0] == 2 || (pSkill.bType[0] == 1 && !isEskrimaSkills())) {
					...
					if (!pRightHand.isnull()) {
						if (pRightHand.isDagger() && pSkill.bType[0] == 2)  
							return SkillUseResult::SkillUseFail;
						if (!isBlowArrowSkills() && pRightHand.isBow() && (pSkill.bType[0] == 1 || pSkill.bType[1] == 1) && pSkill.bCastTime == 0) /*07.12.2023 helendi varken blow arrow kullanılmıyordu fix*/
							return SkillUseResult::SkillUseFail;
					}
				}
...
* 						require.
*/
void MagicInstance::SendSkillFailed(int16 sTargetID, const char* reason)
{
	if (pSkillCaster == nullptr)
		return;
...
bool MagicInstance::IsAvailable()
{
	...
	if (pSkillCaster && pSkillCaster->isPlayer())
	{
		...
		if (pSkill.bType[0] == 1 && pSkillCaster->isPlayer())
		{
			...
			{	// Weapons verification in case of 2 handed attacks !
				...
				if (TO_USER(pSkillCaster)->GetItem(LEFTHAND)->nNum != 0 || (!pRightHand.isnull() && !pRightHand.isSword() && !pRightHand.isAxe() && !pRightHand.isMace() && !pRightHand.isSpear() && !pRightHand.isMace() && !pRightHand.isShield()))
					return false;
			}
		}
		...
		{
			CUser * pCaster = pSkillCaster->isPlayer() ? TO_USER(pSkillCaster) : nullptr;
			...
			{
				CUser * pSkilled = pSkillCaster->isPlayer() ? TO_USER(pSkillCaster) : nullptr;
				...
				&& (pType->bBuffType == BUFF_TYPE_SPEED2 || pType->bBuffType == BUFF_TYPE_SPEED || pType->bBuffType == BUFF_TYPE_STUN))
			{
				...
				if (!te) 
				{
					...
					if (pSkill.iNum == 105520 || pSkill.iNum == 205520 || pSkill.iNum == 106520 || pSkill.iNum == 206520
						|| pSkill.iNum == 106820 || pSkill.iNum == 206820 || pSkill.iNum == 106802 || pSkill.iNum == 206802)
					{
						int32 myrandinfo = myrand(0, 100);
						if (pSkill.bSuccessRate >= myrand(0, 100))
							bSetSpeed = true;
						else
							bSetSpeed = false;
...
short MagicInstance::GetMagicDamage(Unit *pTarget, int total_hit, int attribute)
{
	...
	if (result != FAIL)
	{
		...
		if (pSkillCaster->isPlayer() && pTarget->isNPC() && TO_USER(pSkillCaster)->isWarrior()) 
			damage *= static_cast<int32>(0.50f);
