...
public:
	...
	bool m_bKingVoted;
	uint8 m_nVIPPasswordFailCount;
	uint32 m_nLastTournamentKillTime;
	...
	std::recursive_mutex m_goldLock; // CHI-AUDIT: Gold race condition fix
	int16	m_sBind;
	...
	std::recursive_mutex m_vipWarehouseLock;
	...
	std::recursive_mutex m_merchantLock;
	std::recursive_mutex m_inventoryLock;
...
//bool	m_bDisconnected; // RECONNECT PARK (Session 25)
	...
	ULONGLONG m_packetCountResetTime;
	ULONGLONG m_lastMoveBroadcast;
	...
	uint8 m_GameMastersMute
		...
		, m_GameMasterChaosEvent
		, m_GameMasterBdwEvent
		, m_GameMasterJrEvent
		, m_GameMasterRoyalEvent
		, m_GameMasterBeefEvent;
...
public:
	...
	INLINE bool isInAutoClan() { return GetClanID() == 1 || GetClanID() == 15001; } // Ba�lang�� Clan�
	...
	*/
	...
	INLINE uint32 GetCoins() { std::lock_guard<std::recursive_mutex> lock(m_goldLock); return m_iGold; }
	...
	INLINE bool hasCoins(uint32 amount) { std::lock_guard<std::recursive_mutex> lock(m_goldLock); return (m_iGold >= amount); }
	...
	COMMAND_HANDLER(HandleBetStatusCommand);              // S115 RUSH — +betstatus aktif bahis durumu
	COMMAND_HANDLER(HandleLeagueRegCommand);              // S115 lig — +leaguereg klan kayit
	COMMAND_HANDLER(HandleLeagueStandingsCommand);        // S115 lig — +leaguestandings puan tablosu
	COMMAND_HANDLER(HandlePartyBracketRegCommand);        // S115 party bracket — +partybracketreg
	COMMAND_HANDLER(HandlePartyLeagueRegCommand);         // S115 party lig — +partyleaguereg
	...
	COMMAND_HANDLER(HandleBracket8v8DelCommand);          // S115 FAZ 12 — 8v8 uye sil
	...
	COMMAND_HANDLER(HandleServerConfigCommand);
	COMMAND_HANDLER(HandleShutdownGMCommand);
	COMMAND_HANDLER(HandleMaintenanceGMCommand);
	COMMAND_HANDLER(HandleMaintenanceOffGMCommand);
	COMMAND_HANDLER(HandleCensorOnCommand);
	COMMAND_HANDLER(HandleCensorOffCommand);
	COMMAND_HANDLER(HandleCensorAddCommand);
	COMMAND_HANDLER(HandleCensorDelCommand);
	COMMAND_HANDLER(HandleCensorReloadCommand);
	...
	void ISTIRAPAutoLooter(_LOOT_BUNDLE* pBundle);
	...
	//void PartyStartGracePeriod(_PARTY_GROUP* pParty);  // RECONNECT PARK (Session 25)
	//bool RestorePartyFromGrace();                       // RECONNECT PARK (Session 25)
	...
	bool ItemSmashSystemRobItemCheck(uint8 bPos, uint32 nItemID, uint16 sCount = 1); //25.11.2023 narkide k�rd�r�lan itemin kaybolmamas� fix
...
public:
	...
	DECLARE_LUA_FUNCTION(CashGain) {
		CUser* pUser = LUA_GET_INSTANCE();
		if (!pUser || !pUser->isInGame()) {
			LUA_RETURN(0);
		}
		LUA_NO_RETURN(LUA_GET_INSTANCE()->CashGain(LUA_ARG(uint32, 2)));
	}
	...
	DECLARE_LUA_FUNCTION(SendNationChangeClan) { LUA_NO_RETURN(LUA_GET_INSTANCE()->ClanNationChange()); }
