...
CDBProcess::CDBProcess() {
	for (int i = 0; i < LOGIN_DB_POOL_SIZE; i++)
		m_dbConnections[i] = new OdbcConnection();
}

CDBProcess::~CDBProcess() {
	for (int i = 0; i < LOGIN_DB_POOL_SIZE; i++)
		delete m_dbConnections[i];
}

OdbcConnection* CDBProcess::GetConnection() {
	uint32_t idx = m_poolCounter.fetch_add(1, std::memory_order_relaxed) % LOGIN_DB_POOL_SIZE;
	return m_dbConnections[idx];
}

bool CDBProcess::Connect(string & szDSN, string & szUser, string & szPass)
{
	for (int i = 0; i < LOGIN_DB_POOL_SIZE; i++) {
		if (!m_dbConnections[i]->Connect(szDSN, szUser, szPass)) {
			g_pMain->ReportSQLError(m_dbConnections[i]->GetError());
			return false;
		}
	}
	return true;
...
bool CDBProcess::LoadVersionList()
{
	...
	if (!dbCommand->Execute(_T("SELECT sVersion, sHistoryVersion, strFilename, ISNULL(strFileHash, '') FROM VERSION")))
	{
	...
	if (dbCommand->hasData())
	{
		...
		do
		{
			// L22: Memory leak onlemi — exception durumunda delete
			...
			try {
				...
				dbCommand->FetchString(4, pVersion->strFileHash);
...
bool CDBProcess::LoadServerList()
{
	...
	if (dbCommand->hasData())
	{
		...
		do
		{
			// L23: Memory leak onlemi — exception durumunda delete
...
uint16 CDBProcess::AccountLogin(string & strAccountID, string & strPasswd, std::string & OTP_Key)
{
	...
	if (result == 16)
	{
		OdbcConnection* conn2 = GetConnection();
		unique_ptr<OdbcCommand> dbCommand2(conn2->CreateCommand());
		...
		{
			g_pMain->ReportSQLError(conn2->GetError());
			return 18;
		}
