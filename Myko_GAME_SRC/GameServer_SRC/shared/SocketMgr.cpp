...
uint32 THREADCALL SocketCleanupThread(void * lpParam)
{
	while (SocketMgr::s_bRunningCleanupThread) {
		...
		for (size_t i = 0; i < SocketMgr::s_disconnectionQueue.size(); ) {
			...
			if (pSock->GetSocketMgr() && !pSock->IsDeleted()) {
				...
				SocketMgr::s_disconnectionQueue[i] = SocketMgr::s_disconnectionQueue.back();
				SocketMgr::s_disconnectionQueue.pop_back();
			} else {
				i++;
			}
		}
