...
bool Socket::Connect(const char * Address, uint32 Port)
{
	...
	if (getaddrinfo(Address, portStr, &hints, &result) != 0 || result == nullptr)
		return false;

	memcpy(&m_client, result->ai_addr, result->ai_addrlen);
	freeaddrinfo(result);
...
void Socket::_OnConnect()
{
	...
	// Disable Nagle algorithm — send packets immediately, reduces latency by 40-200ms
...
std::string Socket::GetRemoteIP()
{
	...
	if (inet_ntop(AF_INET, &m_client.sin_addr, ipBuf, sizeof(ipBuf)) != nullptr)
