...
void KOSocket::OnRead()
{
	...
	for (;;)
	{
		...
		if (m_remaining > 65535) { goto error_handler; }

		// Stack buffer for small packets (most packets), heap for large
		uint8 stack_buf[4096];
		uint8* in_stream = (m_remaining <= sizeof(stack_buf)) ? stack_buf : new uint8[m_remaining];
		...
		if (footer != 0xAA55 || !DecryptPacket(in_stream, pkt)) {
			...
			if (in_stream != stack_buf) delete[] in_stream;
			goto error_handler;
		}
...
bool KOSocket::SendCompressed(Packet * pkt)
{
	...
	uint32 inLength = static_cast<uint32>(pkt->size() + 1), outLength = inLength + LZF_MARGIN, crc;
