...
int32 SafeAtoi(const std::string& str, int32 minVal = INT_MIN, int32 maxVal = INT_MAX);
...
INLINE std::string regex_quotes(std::string a) {
	std::string result;
	result.reserve(a.size() + 8);
	for (char c : a) {
		if (c == '\'') result += "''";
		else result += c;
	}
