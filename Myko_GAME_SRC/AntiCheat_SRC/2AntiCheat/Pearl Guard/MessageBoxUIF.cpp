...
void CUIMessageBox::OpenMessageBox(MsgBoxTypes type, ParentTypes parent)
{
	...
	if (Nation == 1)
	{
		std::string name = Engine->dcpUIF(string_format(xorstr("%s\\%s\\Ka_MessageBox.%s"), AcsFolderName, AcsInFolderName, AcsInFolderName));
	...
	{
		std::string name = Engine->dcpUIF(string_format(xorstr("%s\\%s\\El_MessageBox.%s"), AcsFolderName, AcsInFolderName, AcsInFolderName));
		Engine->m_UiMgr->uiMsgBox = new CUIMessageBox(type, parent);
		Engine->m_UiMgr->uiMsgBox->Init(this);
		Engine->m_UiMgr->uiMsgBox->LoadFromFile(Engine->m_BasePath + name.c_str(), N3FORMAT_VER_1068);
		Engine->m_UiMgr->AddChild(Engine->m_UiMgr->uiMsgBox);
...
#else
	std::string name = Engine->dcpUIF(string_format(xorstr("%s\\%s\\re_message_box.%s"), AcsFolderName, AcsInFolderName, AcsInFolderName));
	Engine->m_UiMgr->uiMsgBox = new CUIMessageBox(type, parent);
	Engine->m_UiMgr->uiMsgBox->Init(this);
	Engine->m_UiMgr->uiMsgBox->LoadFromFile(Engine->m_BasePath + name.c_str(), N3FORMAT_VER_1068);
	Engine->m_UiMgr->AddChild(Engine->m_UiMgr->uiMsgBox);
