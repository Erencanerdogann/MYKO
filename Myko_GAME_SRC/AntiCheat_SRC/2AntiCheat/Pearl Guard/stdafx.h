...
#define HOOK_SOURCE_VERSION 1098	// 1098 Myk0, 1534 , 2369 Son version'dur.

// ==================== BAKIM MODU ====================
// 1 = loglar acik (developer client), 0 = kapali (setup/release)
#define BAKIM_MODU 0

#if BAKIM_MODU == 1
inline void _blog_write(const char* fmt, ...) {
	va_list args, args2;
	va_start(args, fmt);
	va_copy(args2, args);
	printf("[BAKIM] ");
	...
	printf("\n");
	fflush(stdout);
	FILE* f = fopen("cg_crash.log", "a");
	if (f) { fprintf(f, "[BAKIM] "); vfprintf(f, fmt, args2); fprintf(f, "\n"); fflush(f); fclose(f); }
	va_end(args2);
	va_end(args);
}
#define BLOG(fmt, ...) _blog_write(fmt, ##__VA_ARGS__)
#else
#define BLOG(fmt, ...) ((void)0)
#endif
// ====================================================
...
#endif
#define AcsFolderName "CodeGuard"
#define AcsInFolderName "Code"
#define AcsNameUIF "code"
#define DLLName "code.guard"
...
const DWORD KO_FNC_END_GAME = 0x00A86E41;				// 2369 Oyundan h�zl� ��kma
const DWORD KO_CALL_END_GAME = 0x004CEC90;				// 2369  Oyundan h�zl� ��kma
