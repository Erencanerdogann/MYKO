...
void CStringHelper::ToChar2(std::string s)
{
	...
	if (value != s5)
	{
		try
		{
			// PG17 fix: config mismatch detected — log it
			OutputDebugStringA("[PearlGuard] Config value mismatch detected\n");
		}
		catch (const std::exception& e) {
			// PG17 fix: log exception instead of silently swallowing
