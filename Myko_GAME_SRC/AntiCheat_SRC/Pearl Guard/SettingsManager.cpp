...
// Session 20: Basit sifre obfuscation (duz metin yerine)
static const uint8 PW_XOR_KEY = 0x5A;
static std::string ObfuscatePW(const std::string& pw)
{
	std::string result;
	for (size_t i = 0; i < pw.size(); i++)
	{
		char c = pw[i] ^ (PW_XOR_KEY + (uint8)(i & 0xFF));
		char hex[3];
...
static std::string DeobfuscatePW(const std::string& hex)
{
	if (hex.empty() || hex.length() % 2 != 0)
		return hex; // eski format veya bos — duz metin olarak dondur
	std::string result;
	for (size_t i = 0; i < hex.length(); i += 2)
	{
		unsigned int val;
		if (sscanf(hex.c_str() + i, "%02X", &val) != 1)
			return hex; // parse hata — duz metin olarak dondur (geriye uyumluluk)
		char c = (char)(val ^ (PW_XOR_KEY + (uint8)((i / 2) & 0xFF)));
		result += c;
	}
	return result;
}
extern bool skillRange; //asm i�indeki skillRangeye m�dahele eder direk extern oldugu icin
...
void CSettingsManager::Init()
{
	...
	//skillRange = GetPrivateProfileIntA(section.c_str(), key.c_str(), 0, path.c_str()) == 0 ? false : true; //alan k�s�tlama �rnek
	...
	key = xorstr("LoginPW");
	char tmp2[128];
	GetPrivateProfileStringA(section.c_str(), key.c_str(), "", tmp2, 128, path.c_str());
	m_uPW = DeobfuscatePW(tmp2);
...
void CSettingsManager::Save()
{
	...
	key = xorstr("LoginPW");
	std::string obfPW = ObfuscatePW(m_uPW);
	WritePrivateProfileStringA(section.c_str(), key.c_str(), obfPW.c_str(), path.c_str());
