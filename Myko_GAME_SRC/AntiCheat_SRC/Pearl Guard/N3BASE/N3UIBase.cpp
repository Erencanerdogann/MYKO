...
void CN3UIBase::Tick()
{
	...
	if (animated)
	{
		if (frameCounter < clock() - (1000 / fps))
		{
			POINT pos = GetPos();
			if (pos.y < endRect.top) {
				POINT to = pos;
				to.x = endRect.left;
				to.y += frame++ / (1000 / fps) * 10;
				if (to.y > endRect.top) to.y = endRect.top;
				SetPos(to.x, to.y);
				frameCounter = clock();
				return;
			}
...
void CN3UIBase::SetupAnimation()
{
	startRect = GetRegion();
	endRect = GetRegion();
	int h = startRect.bottom - startRect.top;
	startRect.top = abs(h) * -1;
	startRect.bottom = startRect.top + abs(h);
}

void CN3UIBase::OpenWithAnimation(uint32 frameTime)
{
	fps = frameTime;
	frame = 0;
	frameCounter = 0;
